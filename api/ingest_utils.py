...
def build_and_index_case_corpus(case_dir: str, case_id: str) -> int:
    """
    Walk the case directory (prefer case_dir/files), convert EVTX + Registry + Prefetch -> text,
    write summaries, and push chunks into Chroma via embed_texts().
    ...
    prefetch_summary_path = os.path.join(case_dir, "prefetch_summaries.jsonl")

    with open(evtx_summary_path, "w", encoding="utf-8") as evtx_summary_f, \
         open(reg_summary_path, "w", encoding="utf-8") as reg_summary_f, \
         open(prefetch_summary_path, "w", encoding="utf-8") as prefetch_summary_f:
        ...
        for root, _, files in os.walk(scan_root):
            for filename in files:
                ...
                # 3) Prefetch
                elif ext in PREFETCH_EXTENSIONS:
                    print(f"[PREFETCH] candidate: {filename}")
                    stats = generate_prefetch_derivatives(path, case_dir)
                    print(f"[PREFETCH] {filename}: {stats['events_count']} entries parsed")
                        ...
                        try:
                            with open(stats["txt_path"], "r", encoding="utf-8", errors="ignore") as f:
                                for line in f:
                                    ...
                                    prefetch_summary_f.write(line + "\n")
                        except Exception as e:
                            print(f"[PREFETCH] failed reading derivative txt for {filename}: {e}")

                # 4) Normal text-like files
